...
function displayMatchItem(item) {
  if (typeof item === 'string' || typeof item === 'number') return String(item);
  if (!item || typeof item !== 'object') return 'Not specified';
  return item.skill || item.name || item.title || item.description || item.message || 'Not specified';
}

function JobCard({ job, active, busy, onSelect }) {
  const analysis = job.analysis;
  return <article className={`job-card ${active ? 'active' : ''}`}><div className="job-card-head"><div><span className="eyebrow">{job.source || 'Platform opportunity'}</span><h3>{job.title}</h3><p><strong>{job.company}</strong> · <MapPin size={13} /> {job.location || 'Location not specified'}</p></div><div className="job-score"><strong>{analysis?.match_score ?? '—'}</strong><span>match</span></div></div>{job.description && <p className="job-description">{job.description}</p>}{job.source_url && <a className="job-source-link" href={job.source_url} target="_blank" rel="noreferrer"><ExternalLink size={14} /> View listing</a>}{analysis && <><ProgressBar value={analysis.readiness_score} label="Readiness" /><div className="matrix-list">{(analysis.strengths || []).slice(0, 2).map((item, index) => <span key={`s-${index}`} className="matrix-row positive"><Check size={14} /> {displayMatchItem(item)}</span>)}{(analysis.gaps || []).slice(0, 2).map((item, index) => <span key={`g-${index}`} className="matrix-row caution"><AlertTriangle size={14} /> {displayMatchItem(item)}</span>)}</div></>}{!analysis && <div className="matrix-row caution"><X size={14} /> Match analysis unavailable</div>}<button type="button" className={active ? 'btn-primary btn-wide' : 'btn-secondary btn-wide'} onClick={onSelect} disabled={busy}>{busy ? 'Selecting…' : active ? 'Selected target' : 'Select and assess'} <ArrowRight size={15} /></button></article>;
}
