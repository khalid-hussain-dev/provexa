...
def get_two_factor_verification_user(
    ...
    """Resolve a user who may verify either enrollment or a pending login.

    A standard authenticated token is valid while a user confirms a newly
    generated authenticator secret. A pending 2FA token is valid only for the
    login-completion case. This mirrors the Platform authentication contract
    while preserving the Integration host's server-side session checks.
    """
