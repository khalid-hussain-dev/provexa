...
`python -m integration.main` starts Uvicorn on `0.0.0.0:${PORT:-8000}`. For
local use it loads the ignored workspace `.env` without overriding variables
already supplied by the shell. It refuses an unresolved `CHANGE_ME` database
credential before accepting requests.
