...
- Experience Builder adapter tests: 4 passed, including live-route and TOTP enrollment coverage.
- Experience Builder production build: passed.
- Integration tests: 28 passed, including composed-host TOTP enrollment and later-login challenge coverage.
...
The copied frontend contains deliberate integration-only hardening after the
initial baseline copy: user-facing TOTP setup, friendly live-host failure
diagnostics, a workspace error boundary, robust adapter-value rendering, and a
deployment API-origin setting. The root Experience Builder baseline remains
unchanged; see `EXPERIENCE_INTEGRATION_DELTA.md`.
